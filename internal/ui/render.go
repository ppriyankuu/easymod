...
func (p *Permissions) renderPermissions() string {
	...
	for _, v := range p.values {
		...
		{
			if len(ownerBlock) < 1 {
				ownerBlock = append(ownerBlock, styles.PermissionsBlockItem.Foreground(lipgloss.Color(ColorYellow)).Render("[Owner]"))
		...
		{
			if len(groupBlock) < 1 {
				groupBlock = append(groupBlock, styles.PermissionsBlockItem.Foreground(lipgloss.Color(ColorYellow)).Render("[Group]"))
		...
		{
			if len(otherBlock) < 1 {
				otherBlock = append(otherBlock, styles.PermissionsBlockItem.Foreground(lipgloss.Color(ColorYellow)).Render("[Other]"))
	...
	blocks := lipgloss.JoinHorizontal(
		...
		groupBlockStyle.Render(lipgloss.JoinVertical(
			lipgloss.Left,
			groupBlock...,
		)),
		otherBlockStyle.Render(lipgloss.JoinVertical(
			lipgloss.Left,
			otherBlock...,
		)),
	)
